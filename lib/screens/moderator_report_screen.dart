...
class _ModeratorReportsScreenState extends State<ModeratorReportsScreen> {
  ...
  Future<void> updateReportStatus(Report report, String newStatus) async {
    try {
      final res = await http.post(
        Uri.parse('${baseUrl}report_update.php'),
        body: {'id': (report.id ?? 0).toString(), 'status': newStatus},
      );
  ...
  Future<void> deleteReport(Report report) async {
    ...
    try {
      final res = await http.post(
        Uri.parse('${baseUrl}report_delete.php'),
        body: {'id': (report.id ?? 0).toString()},
      );
      if (res.statusCode == 200) {
        final data = json.decode(res.body);
        if (data['success'] == true) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: const LeopardAppBar(title: Text('Report Management')),
      ...
      body: Column(
        children: [
          Container(
            ...
            child: Row(
              children: [
                Text('Filter by status:', style: theme.textTheme.bodyMedium),
...
class ReportCard extends StatelessWidget {
  ...
  final String imageBaseUrl = 'http://0.0.0.0:3441';
  ...
  Widget build(BuildContext context) {
    final hasImage =
        report.reportedImageUrl != null && report.reportedImageUrl!.isNotEmpty;
                ...
                Wrap(
                  ...
                  children: [
                    ...
                    _buildLabel(
                      ...
                      getStatusColor(context).withOpacity(0.2),
          ...
          Padding(
            ...
            child: Column(
              ...
              children: [
                ...
                Text(report.reportedContent, style: theme.textTheme.bodyMedium),
              ],
            ),
          ),
        ],
  ...
  // Helper method to build status labels
  Widget _buildLabel(
    String text,
  ...
  Widget _buildMobileButtons(BuildContext context) {
    final theme = Theme.of(context);
    final smallButtonStyle = TextStyle(color: theme.colorScheme.primary);
    return Column(
      ...
      children: [
        ...
        if (report.status == 'pending') ...[
          retro.RetroButton(
            onTap: () => onStatusChange('reviewed'),
            child: Text('REVIEWED', style: smallButtonStyle),
          ),
          const SizedBox(height: 8),
          retro.RetroButton(
            onTap: () => onStatusChange('dismissed'),
            child: Text('DISMISS', style: smallButtonStyle),
          ),
          const SizedBox(height: 8),
        ],
        retro.RetroButton(
