...
try {
    ...
    if (!$stmt) {
        throw new Exception(
            "Database prepare statement failed: " . $conn->error,
        );
    }
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($row["report_type"] === "thread") {
            $row["reported_content"] =
                $row["thread_title"] ?? "[Thread not found or deleted]";
        } elseif ($row["report_type"] === "reply") {
            $row["reported_content"] =
                $row["post_content"] ?? "[Reply not found or deleted]";
        } else {
        ...
        $row["reported_image_path"] =
            $row["thread_image_path"] ?? ($row["reply_image_path"] ?? null);

        // Clean up unnecessary fields
        unset(
            $row["thread_title"],
            $row["post_content"],
            $row["thread_image_path"],
            $row["reply_image_path"],
        );
